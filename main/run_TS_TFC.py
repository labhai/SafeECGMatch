...
sys.path.append("./")
from configs import TSTFCConfig
from datasets.cinc2021 import get_cinc2021
from datasets.ptbxl import get_ptbxl
from models.resnet1d import ResNet1D
from tasks.classification_TS_TFC import Classification
...
def main():
    config = TSTFCConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    if config.data not in NUM_CLASSES:
        raise NotImplementedError("TS_TFC currently supports ECG datasets only.")
    if config.backbone_type != "resnet1d":
        raise NotImplementedError("TS_TFC currently supports resnet1d only.")
